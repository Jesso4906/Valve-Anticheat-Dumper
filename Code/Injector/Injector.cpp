...
int main()
{
    std::string dllPath = GetDLLPath("VACModuleDumper.dll");

    HANDLE processHandle = GetProcessHandle(L"SteamService.exe", true);
    ...
    if (succeeded) 
    {
        ...
        std::cout << "Run any VAC game and play for a while so that all modules can be dumped\n";
        std::cout << "VAC modules will be dumped at C:\\VACDumps\n";
        Sleep(10000);
    }
    else 
    {
